...
var an_CndTempRaw = 0.00;
var an_AmbTempRaw = 0.00;
var an_SwSetTemp1 = 1.23;
var an_SwSetTemp2 = 0.00;
var ctltype = "OFF";
var bSetPLock = false;
var ReqCount =0;
var strCTLTYPE = "OFF";
var strLstCTLTYPE = "OFF";

/*<!-- Start Gauge source code HTML -->*/
...
/*<!-- Sets Button color based on value OFF\ON and Color RED\GREEN -->*/
function bgcolorset(strajx,btnid)
{
	var strSch = "";
	if(btnid == "btnaux")
	{
	strSch = "AUXFAN: TRUE";
	}else if(btnid == "btnacmn"){
	strSch = "ACMAIN: TRUE";
	}else{
	strSch = "INVALID";
}
	/*<!-- if we find true, then btn bg should be green -->*/
	if(strajx.search(strSch)>-1)
	{
		document.getElementById(btnid).style.background = '#00ff00';
		document.getElementById(btnid).value = 'ON';
	}else{
		document.getElementById(btnid).style.background = '#ff0000';
		document.getElementById(btnid).value = 'OFF';
	}
}
		...
		if (this.readyState == 4) {
			if (this.status == 200) {
				if (this.responseXML != null) {
					...
					an_SwSetTemp1 = this.responseXML.getElementsByTagName('an_SetTemp1')[0].childNodes[0].nodeValue;
					an_SwSetTemp2 = this.responseXML.getElementsByTagName('an_SetTemp2')[0].childNodes[0].nodeValue;
					/*<!-- Set Software Demand Temperature set point 1 -->*/
					document.getElementById("an_swset1").innerHTML =
					this.responseXML.getElementsByTagName('an_SetTemp1')[0].childNodes[0].nodeValue;
					/*<!-- Set Software Demand Temperature set point 2 -->*/
					document.getElementById("an_swset2").innerHTML =
					this.responseXML.getElementsByTagName('an_SetTemp2')[0].childNodes[0].nodeValue;
					if(!bSetPLock){
						document.getElementById("txt_setpoint1").form_textsetpoints1.value = an_SwSetTemp1;
					}
				}
			}else{
				an_CndTempRaw = 1000;
				...
				an_SwSetTemp1 = 1000;
				an_SwSetTemp2 = 1000;
				document.getElementById("an_swset1").innerHTML = "HTTP Conn Error";
				document.getElementById("an_swset2").innerHTML = "HTTP Conn Error";
				if(!bSetPLock){
					document.getElementById("txt_setpoint1").form_textsetpoints1.value = 99.99;
					console.log("Set Points Unlocked");
				}else{
					//document.getElementById("txt_setpoint1").form_textsetpoints1.value = 97.97;
					console.log("Set Points Locked");
				}
				console.log("GetGaugeData XML REQ Response Error: " + request.status +"\r\n"); // An error occurred during the request.
			}
		}
	}
...
//<!-- Sends XML GET request for some digital and analog parameters, TSTAT DEMAND, COND FROST Fault,COND temperature,AMBIENT ROOM temperature-->
function GetXml()
{
	nocache = "&nocache=" + Math.random() * 1000000;
	var request = new XMLHttpRequest();
	request.onreadystatechange = function()
	{
		//<!-- If response in DONE '4' && successful '200' -->
		if (this.readyState == 4) {
			if (this.status == 200) {
				if (this.responseXML != null) {
					//<!-- extract data from the XML file, tag names -->
					...
					document.getElementById("dig_ACSTATE").innerHTML =
					this.responseXML.getElementsByTagName('dig_ACSTATE')[0].childNodes[0].nodeValue;
					
					//update radio button
					strCTLTYPE = document.getElementById("dig_ACSTATE").innerHTML;
					if(strCTLTYPE != strLstCTLTYPE){
						setRadioVal(document.getElementById("ctltypeForm"), "ctltype", strCTLTYPE);
					}
					strLstCTLTYPE = strCTLTYPE;
				}
			}else{
				document.getElementById("dig_1").innerHTML = "HTTP Conn Error";
				document.getElementById("dig_2").innerHTML = "HTTP Conn Error";
				document.getElementById("an_1").innerHTML = "HTTP Conn Error";
				document.getElementById("an_2").innerHTML = "HTTP Conn Error";
				document.getElementById("dig_ACSTATE").innerHTML = "HTTP Conn Error";
				
				console.log("GetXml XML REQ Response Error: " + request.status +"\r\n"); // An error occurred during the request.
			}
		}
	}
	...
	bgcolorset(document.getElementById("dig_1").innerHTML,"btnaux");
	bgcolorset(document.getElementById("dig_1").innerHTML,"btnacmn");
	GetGaugeData();//call get gauge data function
}
//<!-- Toggles button value OFF\ON and Color RED\GREEN -->
function colorchange(id,ssr)
{ 
	...
	xhr.onreadystatechange = function () {
		...
		if (xhr.readyState === DONE) {
			if (xhr.status === OK) {
				console.log("SSR Manual " +ssr +"XML REQ Response: " +xhr.responseText +"\r\n"); // 'This is the returned text.'
			} else {
				console.log("SSR Manual " +ssr +"XML REQ Response Error: " + xhr.status +"\r\n"); // An error occurred during the request.
			}
		}
	}
	...
	{ 
		xhr.open("GET",ssr+"=1",true);	
	}
	else { 
		xhr.open("GET",ssr+"=0",true);
	}
	xhr.send();
}

//<!-- Sends the Control set point over to the web server -->
function SendSetPoint()
{
	...
	xhr.onreadystatechange = function () {
		...
		if (xhr.readyState === DONE) {
			if (xhr.status === OK) {
				console.log("Send SetP XML REQ Response: " +xhr.responseText +"\r\n"); // 'This is the returned text.'
			} else {
				console.log("Send SetP XML REQ Response Error: " + xhr.status +"\r\n"); // An error occurred during the request.
			}
			//<!-- Unlock Text Area After Set Points Are sent -->
			bSetPLock = false;
		}
	}
	
	strText = document.getElementById("txt_setpoint1").form_textsetpoints1.value;
	xhr.open("GET", "ajax_chgsetpoint_" + strText +"_end" + nocache, true);
	xhr.send(null);
}

//<!-- Check value passed in and set variable to indicate control mode selected -->
function ctlradiochanged(radio)
{
    if(radio.value == "TSTAT"){
		ctltype = "TSTAT";
	...
	console.log("Control Type: " + ctltype + "\r\n");
	...
	xhr.onreadystatechange = function () {
		...
		if (xhr.readyState === DONE) {
			if (xhr.status === OK) {
				console.log("XML REQ Response: " +xhr.responseText); // 'This is the returned text.'
				strCTLTYPE = xhr.responseText;			
			} else {
				console.log('XML REQ Response Error: ' + xhr.status); // An error occurred during the request.
				strCTLTYPE = "ERROR";
			}
	...
	xhr.open("GET", "ajax_chgctlmode_" + ctltype +"_end" + nocache, true);
	xhr.send(null);
}

//<!-- Set checked value to indicate control mode selected -->
function setctlradio(radio, val)
{
	radio.value = val;
}

function getCheckedRadioVal(form, name) {
    var val;
...
function setRadioVal(form, name, text) {
    var retval = false;
    // get list of radio buttons with specified name
    var radios = form.elements[name];
    ...
    // loop through list of radio buttons
    for (var i=0, len=radios.length; i<len; i++) {
        if ( radios[i].value == text ) { // look for value
            radios[i].checked = true; // if so, check it
			retval = true;
            break; // and break out of for loop
        }
    }
    return retval; // return value of checked radio or undefined if none checked
}


function LockSetPTextArea(txta) {
    bSetPLock = true;
	txta.value = "";
}